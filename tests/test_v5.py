"""
Tests for v5_skills_agent_demo/skills_agent.py - Dynamic skill injection.
...
from tests.helpers import SKILL_TOOL

os.environ.setdefault("LLM_API_KEY", "test-key")
os.environ.setdefault("LLM_BASE_URL", "https://api.openai.com/v1")
os.environ.setdefault("LLM_MODEL", "gpt-4o-mini")

from v5_skills_agent_demo.skills_agent import (
    SkillLoader,
    run_skill,
    SYSTEM_PROMPT as SYSTEM
)

V5_TOOLS = [BASH_TOOL, READ_FILE_TOOL, WRITE_FILE_TOOL, EDIT_FILE_TOOL, SKILL_TOOL]
...
def test_skill_injection_mechanism():
    ...
    v5's unique value: skill body goes into tool_result (user message) via
...
def test_run_skill_unknown_returns_error():
    """run_skill should return clear error text for unknown skills."""
    missing_skill_name = "__definitely_missing_for_test__"
    result = run_skill(skill_name = missing_skill_name)
    assert result.startswith("Error: Unknown skill"), (
        f"Expected unknown skill error, got: {result}"
    )
    assert missing_skill_name in result, "Error should include the missing skill name"
    print("PASS: test_run_skill_unknown_returns_error")
    return True
...
def test_llm_loads_skill():
    ...
    text, calls, _ = run_agent(
        client,
        "Call the Skill tool with skill_name='test-skill' to load it.",
        tools,
        system = "You MUST use the Skill tool when asked to load a skill. "
        "Call Skill with the skill_name parameter.",
    )
...
def test_llm_follows_skill_instructions():
    ...
    text, calls, _ = run_agent(
        client,
        "Call the Skill tool with skill_name='test-skill'. Then summarize the steps it recommends.",
        tools,
...
if __name__ == "__main__":
    sys.exit(0 if run_tests([
        ...
        test_run_skill_unknown_returns_error,
