"""
v1 test suite: "Bash is everything" -- the simplest possible agent, only bash tool.
...
from tests.helpers import BASH_TOOL
...
def test_bash_echo():
    ...
    response, calls, _ = run_agent(
        client,
        "Run 'echo hello_from_v1' using the bash tool and tell me the exact output.",
        V1_TOOLS,
    )

    assert len(calls) >= 1, "Should make at least 1 tool call"
    assert any(c[0] == "bash" for c in calls), "Should use bash tool"
    assert response and "hello_from_v1" in response, (
        f"Response should contain 'hello_from_v1', got: {response[:200]}"
    )
...
def test_bash_subagent_spawn():
    """Model spawns a subagent via 'python v1_bash_agent_demo/bash_agent.py' to list files."""
    client = get_client()
    if not client:
        print("SKIP: No API key")
        return True
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        response, calls, _ = run_agent(
            client,
            f"Spawn a subagent using the command 'python v1_bash_agent_demo/bash_agent.py' to list files in {tmpdir}. "
            f"You MUST call bash with a command that includes 'python' and 'v1_bash_agent_demo/bash_agent.py' in it.",
            V1_TOOLS,
            workdir = tmpdir,
            max_turns = 5,
        )
        ...
        has_subagent_call = any(
            "python" in cmd and "v1_bash_agent_demo/bash_agent.py" in cmd for cmd in bash_cmds
        )
        assert has_subagent_call, (
            f"Should call bash with a command containing 'python' and 'v1_bash_agent_demo/bash_agent.py', "
...
def test_bash_output_truncation():
    ...
    v1_bash_agent_demo/bash_agent.py truncates at 50000 (line 174: output[:50000]).
