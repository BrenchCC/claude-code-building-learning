...
pypdfium2 是 Chromium 的 PDF 库 PDFium 的 Python 绑定。它非常适合快速的 PDF 渲染、图像生成，并可作为 PyMuPDF 的替代品。
        ...
        for pdf_file in pdf_files:
            try:
                ...
                with open(output_file, 'w', encoding='utf-8') as f:
                    f.write(text)
                logger.info(f"从 {pdf_file} 中提取文本")

            except Exception as e:
                logger.error(f"从 {pdf_file} 中提取文本失败: {e}")
                continue
```
...
- `pdftotext -bbox-layout` 是提取纯文本最快的方法
